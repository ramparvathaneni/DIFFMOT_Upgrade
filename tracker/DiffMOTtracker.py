...
class diffmottracker(object):
    ...
    def update(self, dets_norm, model, frame_id, img_w, img_h, tag, img=None):
        ...
        # FIXED: Proper shape checking for embedding cost
        emb_cost = None
        if trk_embs.shape[0] > 0 and dets_embs.shape[0] > 0:
            emb_cost = trk_embs @ dets_embs.T
        ...
        if min(iou_matrix.shape) > 0:
            ...
            if a.sum(1).max() == 1 and a.sum(0).max() == 1:
                ...
                # FIXED: Check shape compatibility before combining matrices
                if emb_cost is None or emb_cost.shape != iou_matrix.shape:
                    final_cost = -iou_matrix
                else:
        ...
        for m in matched_indices:
            if m[0] < iou_matrix.shape[0] and m[1] < iou_matrix.shape[1]:
