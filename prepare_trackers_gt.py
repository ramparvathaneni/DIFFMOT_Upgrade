...
label_root = 'DanceTrack/trackers_gt'
mkdirs(label_root)
trainer = ["train1"]

for tr in trainer:
    seq_root_tr = (osp.join(seq_root, tr))
    info_root_tr = (osp.join(seq_root, 'train1'))
    seqs = [s for s in os.listdir(seq_root_tr) if osp.isdir(osp.join(seq_root_tr, s))]
    seqs.sort()
...
print("✓ Training data prepared in DanceTrack/trackers_gt/")
